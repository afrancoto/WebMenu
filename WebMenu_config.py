...
##########     
    def paypal(self, widget, data=None):
    	webbrowser.open("https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=XRW3Y7GL8BLPQ")
...
##########  
    def reset_to_default(self, widget, liststore, all_album_check, all_artist_check, options_check, llyrics_check):
	global services, services_order, other_settings, shortcuts
	...
	if response == Gtk.ResponseType.YES:
		...
		llyrics_check.set_active(other_settings[3])
...
##########  
	...
	all_artist_check = Gtk.CheckButton("Artist/All") #The 'All'-artist checkbox
        all_artist_check.set_active(other_settings[2])
	all_artist_check.connect("toggled", self.other_settings_toggled, 2) #The last argument, 2, stands for "Artist"   
	...
	options_check = Gtk.CheckButton("Web/Options") #The 'Options' checkbox
        options_check.set_active(other_settings[0])
	options_check.connect("toggled", self.other_settings_toggled, 0) #The last argument, 0, stands for the "Options" item
	other_settings_hbox.pack_start(options_check, False, True, 10)

	llyrics_check = Gtk.CheckButton("Lyrics") #The lyrics checkbox
        llyrics_check.set_active(other_settings[3])
	llyrics_check.connect("toggled", self.other_settings_toggled, 3) #The last argument, 3, stands for "lyrics"   
	other_settings_hbox.pack_start(llyrics_check, False, True, 0)
	...
	hbox1.pack_start(new_button, False, True, 0)
        ...
        reset_button.connect("clicked", self.reset_to_default, liststore, all_album_check, all_artist_check, options_check, llyrics_check)
	...
	contribute_button = Gtk.Button("Contribute with a little donation! :D")
	contribute_button.connect("clicked", self.paypal)
        hbox2.pack_start(contribute_button, True, True, 0)
